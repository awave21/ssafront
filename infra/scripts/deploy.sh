...
Режимы:
  backend   Пересобрать и перезапустить api + sqns-sync-worker + script-flow-index-worker + scenario-delayed-worker (db/redis поднимутся автоматически)
  frontend  Пересобрать и перезапустить frontend
  worker    Пересобрать и перезапустить sqns-sync-worker + script-flow-index-worker + scenario-delayed-worker
...
case "$TARGET" in
  backend)
    ...
    "${DC[@]}" up -d db redis
    "${DC[@]}" build api sqns-sync-worker script-flow-index-worker scenario-delayed-worker
    if [[ "$RUN_MIGRATIONS" -eq 1 ]]; then
      "$MIGRATIONS_SCRIPT"
    fi
    "${DC[@]}" up -d api sqns-sync-worker script-flow-index-worker scenario-delayed-worker
    ;;
  frontend)
    echo "Деплой frontend: пересборка frontend + landing + проверка воркеров"
    "${DC[@]}" up -d --build frontend landing
    "${DC[@]}" up -d sqns-sync-worker script-flow-index-worker scenario-delayed-worker
    ;;
  worker)
  ...
  all)
    ...
    "${DC[@]}" up -d db redis
    "${DC[@]}" build api frontend landing sqns-sync-worker script-flow-index-worker scenario-delayed-worker
    if [[ "$RUN_MIGRATIONS" -eq 1 ]]; then
      "$MIGRATIONS_SCRIPT"
    fi
    "${DC[@]}" up -d api frontend landing sqns-sync-worker script-flow-index-worker scenario-delayed-worker
