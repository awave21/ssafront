<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <h3 class="text-sm font-bold uppercase tracking-widest text-slate-400">Топ категорий</h3>
        ...
        <div class="relative flex h-full flex-col">
          ...
          <div class="group/title relative mb-6">
            ...
            <p class="text-[10px] font-medium text-slate-400 mt-1">
              {{ category.services_count }} услуг в категории
            </p>
          </div>
    ...
    >
      ...
      <p class="text-sm font-medium text-slate-500">Нет данных для формирования топа категорий</p>
    </div>
  </div>
</template>
...
import type { AnalyticsServiceTableItem } from '~/types/analytics'
...
const topCategories = computed(() => {
  ...
  props.items.forEach(item => {
    const catName = item.service_category || 'Без категории'
