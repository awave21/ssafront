        ...
        <template v-else-if="items.length">
          ...
          >
            <TableCell
              v-for="column in orderedColumns"
              :key="column.id"
              :class-name="column.id === 'service_name' ? 'font-bold text-slate-900' : 'text-slate-600'"
              :style="columnCellStyle(column.id)"
            >
              <template v-if="column.id === 'service_name'">
                <div class="flex items-center gap-3">
                  ...
                  <div class="flex flex-col min-w-0">
                    <span class="truncate">{{ item.service_name }}</span>
                    <span v-if="item.service_category" class="text-[9px] font-bold uppercase tracking-tight text-slate-400">
                      {{ item.service_category }}
                    </span>
                  </div>
                  <Badge
                    v-if="item.service_name.toLowerCase() === 'не определено'"
                    variant="secondary"
      ...
      <TableFooter v-if="totals && !loading">
        <TableRow class-name="hover:bg-transparent">
          <TableCell
            v-for="column in orderedColumns"
            :key="column.id"
            :class-name="column.id === 'service_name' ? 'font-semibold text-slate-900' : undefined"
            :style="columnCellStyle(column.id)"
          >
...
import type { AnalyticsServiceTableItem, AnalyticsServicesTableTotals, ServicesTableSortBy, ServicesTableSortOrder } from '~/types/analytics'
...
const STORAGE_KEY = 'analytics-services-table-columns-v1'
const MIN_COLUMN_WIDTH = 90

const DEFAULT_COLUMNS: ColumnDef[] = [
  { id: 'service_name', label: 'Услуга', sortBy: 'service_name' },
  { id: 'service_category', label: 'Категория' },
  ...
  { id: 'payments_total', label: 'Транзакции', title: 'Число строк оплат по этой услуге (один визит с несколькими услугами = несколько строк)' },
  { id: 'avg_check', label: 'Ср. чек (дошедший)', sortBy: 'avg_check', title: 'Выручка по услуге / число дошедших пациентов' },
  { id: 'share_bookings', label: 'Доля записей %' },
]

const DEFAULT_WIDTHS: Record<ColumnId, number> = {
  service_name: 260,
  service_category: 170,
  bookings_total: 120,
  arrived_total: 120,
  primary_total: 120,
...
const props = defineProps<{
  ...
  totals: AnalyticsServicesTableTotals | null
  totalItems: number
  currentPage: number
  totalPages: number
  sortBy: ServicesTableSortBy
...
const emit = defineEmits<{
  (e: 'sort-change', sortBy: ServicesTableSortBy): void
...
const emitSort = (sortBy: ServicesTableSortBy) => emit('sort-change', sortBy)
const sortIndicator = (column: ServicesTableSortBy) => {
...
const renderItemCell = (item: AnalyticsServiceTableItem, columnId: ColumnId) => {
  switch (columnId) {
