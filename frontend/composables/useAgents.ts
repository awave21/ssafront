...
export type SqnsStatus = {
  sqnsEnabled: boolean
  sqnsHost?: string | null
  sqnsCredentialId?: string
  sqnsLastSyncAt?: string
  sqnsStatus?: 'ok' | 'error' | string | null
  sqnsError?: string | null
...
export type Agent = {
  ...
  knowledge_tool_description?: string | null
  status: AgentStatus
  is_disabled: boolean
  version: number
  created_at: string
...
export const useAgents = () => {
  ...
  const updateSqnsTool = async (
    ...
    if (!hasEnabled && !hasDescription) {
      throw new Error('Нужно передать enabled или description')
    }
    ...
    try {
      ...
      const response = await apiFetch<SqnsStatus>(`/agents/${agentId}/sqns/tools/${toolName}`, {
        method: 'PATCH',
        body: data
      })
      ...
      sqnsError.value = getReadableErrorMessage(err, 'Не удалось обновить настройки SQNS-инструмента')
      throw err
    } finally {
      isSqnsLoading.value = false
    }
